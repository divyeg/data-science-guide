...
def classification_model_performance_report(y, y_hat, y_hat_proba, title):
    """
    This function is used to summarize model performance report for classification model

    Params:
    ----------------
    y: true y values -> bool
        pd.Series or numpy.ndarray
    y_hat: prediction y values -> bool
        pd.Series or numpy.ndarray
    y_hat_proba: predicted probabilities for y
        pd.Series or numpy.ndarray
    ...
    average_precision = np.round(average_precision_score(y, y_hat), 4)
    accuracy = np.round(accuracy_score(y, y_hat), 4)
    balanced_accuracy = np.round(balanced_accuracy(y, y_hat), 4)
    mathews_cor = np.round(mathews_corrcoef(y, y_hat), 4)
...
def plot_categorical_countplots(
    column_x1, column_x2, column_x3, column_x4, hue, df, figsize=(12, 12)
):
    """
